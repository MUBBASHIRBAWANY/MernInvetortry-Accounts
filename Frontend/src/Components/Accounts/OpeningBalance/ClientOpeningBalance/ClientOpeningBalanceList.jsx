...
const ClientOpeningBalanceList = () => {
    ...
    console.log(editOpeningClient)

    const navigate = useNavigate()
    const getData = async () => {
        const data = await getDataFundtion("/ClientOpeningBalance")
        const list = await getDataFundtion("/customer")
        dispatch(fetchClient(list.data))
        dispatch(fetchClientOpeningReducer(data.data))
        setRows(data.data)
    ...
    const handleEditClick = (id) => {
        const check = editOpeningClient.find((item) => item._id == id).Status

        if (check == "false") {
            navigate(`/ClientOpeningEdit/${id}`)
        }
        else {
    ...
    const handleViweClick = (id) => {
        console.log(id)
        navigate(`/ClientOpeningBalanceView/${id}`)
    }
    const handleConfirmDelete = async () => {
        ...
        const res = deleteDataFunction(`/ClientOpeningBalance/deleteOpening/${selectedId}`)
    ...
    const columns = [
        ...
        {
            field: 'ClientData', headerName: 'Total Inventory', width: 200,
            renderCell: (params) => {
...
export default ClientOpeningBalanceList