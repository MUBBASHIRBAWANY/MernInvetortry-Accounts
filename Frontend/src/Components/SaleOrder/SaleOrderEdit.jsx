    ...
    const getData = () =>{
        ...
        reset({
            ...
            salesInvoice : SaleOrder.SaleOrderNumber

        })
        setEditOrder(SaleOrder)
        setSalesLoction(SaleOrder.Location)
        setDrp(SaleOrder.Location)
        const defultClient = Client.filter((item)=> item._id === SaleOrder.Customer ).map((val)=> ({
            value : val._id,
            label : val.CutomerName
            
        }))
        console.log(defultClient)
        setPoClient(defultClient)
        setSalesStore(SaleOrder.Store)
        setOderBooker(SaleOrder.OrderBookerId)    
        setTableData(SaleOrder.SaleOrderData)
    }
    ...
    useEffect(()=>{
        ...
        console.log(data)
            try {
                ...
                console.log(res)
                toast.success("Sales Invoice Edit")
                setTimeout(() => {
        ...
        return (
            <div className="p-4  ">
                <ToastContainer />
                <h1 className="text-xl md:text-3xl font-bold text-center text-gray-800 mb-6">Edit Sales Order</h1>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-4 md:p-6 rounded-lg shadow-md">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <Select  isDisabled={true} onChange={(v) => setClient(v)} options={OrderBookerDrp} value={OrderBookerDrp.filter((item)=> item.value === editOrder.OrderBookerId)} />
                        </div>
                        <div>
                        ...
                        <div>
                            <label className="block text-sm md:text-base text-gray-700 font-semibold mb-2">Order No</label>
                            <input
                                type="text"
                                disabled
                                defaultValue={''}
                        ...
                        <div>
                            ...
                            <Select onChange={(v) => setDrp(v)} options={Location} value={Location.filter((item)=> item.value === salesLoction).map((val)=> ({
                                label : val.label,
                                value : val._id
                            }))} />
                        </div>
                        <div>
                            ...
                            <Select onChange={(v) => setSalesStore(v)}  options={storeDrp} value={storeDrp.filter((item)=> item.value == salesStore).map((val)=> ({
                                label : val.label,
                                value : val._id
                            }))} />
                        </div>
