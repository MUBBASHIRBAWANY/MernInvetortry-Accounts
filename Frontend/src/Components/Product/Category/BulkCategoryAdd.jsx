...
const BulkCategoryAdd = () => {
  ...
  const onSubmit = async () => {
    ...
    const Values = data.map((item, index) => ({
      ...
      vendor: Vendor.find((Item) => Item.salesFlowRef == item.vendor)._id,
      vendorCode: Vendor.find((vendorItem) => vendorItem.salesFlowRef == item.vendor).code,
      salesFlowRef: item.salesFlowRef,
      CodeRef  : Vendor.find((Item) => Item.salesFlowRef == item.vendor).ShortCode + item.salesFlowRef
    }));
      console.log(Values)

    try {
      console.log(Values)
      const res = await axios.post("http://localhost:3000/Category/PushBulkData", Values)
...
export default BulkCategoryAdd