...
const BulkBrandAdd = () => {
  ...
  const onSubmit = async () => {
    ...
    const Values = data.map((item) => ({
      ...
      category: category.find((categoryItem) => categoryItem.CodeRef == item.Vendor + item.CategoryName)?._id,
      mastercode: category.find((categoryItem) => categoryItem.CodeRef == item.Vendor + item.CategoryName)?.mastercode,
...
console.log( chunks)
      try {
        for (const chunk of chunks) {
          const res = await createDataFunction("/Brand/PushBulkData", chunk);
          console.log(res)
        }
        toast.success("All data added successfully");
        setTimeout(() => navigate('/brandList'), 2000);
      } catch (err) {
