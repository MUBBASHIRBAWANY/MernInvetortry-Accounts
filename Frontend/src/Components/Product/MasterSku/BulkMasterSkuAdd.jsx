...
const BulkMasterSkuAdd = () => {
  ...
  const onSubmit = async () => {
    ...
    const Values = data.map((item) => ({
      MasterSkuName: item.MaskuName,
      Brand: Brand.find((Branditem) => Branditem.CodeRef == item.Vendor+item.Brand)._id,
      mastercode: Brand.find((Branditem) => Branditem.CodeRef == item.Vendor+item.Brand).mastercode,
    ...
    console.log( chunks)
          try {
            for (const chunk of chunks) {
              const res = await createDataFunction("/MasterSku/PushBulkData", chunk);
              console.log(res)
            }
            toast.success("All data added successfully");
            setTimeout(() => navigate('/masterskulist'), 2000);
          } catch (err) {
...
export default BulkMasterSkuAdd