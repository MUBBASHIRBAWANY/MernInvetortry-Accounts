...
console.log(OrderDc)
    const OrderDcOptions = OrderDc.filter((item) => item.Status == "true").map((item) => {
        return ({
            value: item.DcNumber,
            label: item.DcNumber
        })
    })
console.log(OrderDcOptions)
    const handleSubmit = async (e) => {
        ...
        const allTrue = filtervalues.filter((item) => item.Status == "true").map((item) => {
            return ({
                id: item._id,
                inv: item.DcNumber,
                date: item.DcDate,
...
export default OrderDCUnPostModal;