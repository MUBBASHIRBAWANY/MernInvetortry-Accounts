  ...
  const [data, setData] = useState([]);
    ...
    const onSubmit = async () => {
      ...
      const Values = data.map((item, index) => ({
        ChanneName: item.ChannelName,
        ChanneType: ChannelType.find((item1)=> item1.ChanneTypeName  === item.ChannelType)?._id,
      }));
      
      try {
        console.log(Values)
        const res = await createDataFunction("/Channel/PushBulkData", Values)
