...
const TransferOutAdd = () => {
    ...
    const onSubmit = async (data) => {
        data.LocationFrom = loginVendor.userType == 1 ? selectedLocationFrm : selectedLocationFrm.value
        data.StoreFrom = loginVendor.userType == 1 ? selectedStoreFrom : selectedStoreFrom.value
        data.LocationTo = loginVendor.userType == 1 ? selectedLocationTo : selectedLocationTo?.value
        data.StoreTo = loginVendor.userType == 1 ? selectedStoreTo : selectedStoreTo.value
        ...
        try {
            ...
            if (rus.length != 0) {
                ...
                const res = await createDataFunction("/TransferOut", data)
                console.log(res)
    ...
    const allLocation = Location.map((item) => ({
        label: item.LocationName,
        value: item._id 
    }))
    ...
    const setDrp2 = (value) => {
        ...
        const values = value.value;
        setstoreToDrp([]);
        console.log(loginVendor.Store[0])
        if (loginVendor.userType != 1) {
            ...
            console.log("first")
            const updatedStoreDrp = Store?.filter(item => loginVendor.Location[0].includes(item.Location))
                .filter(store => !loginVendor.Store[0].includes(store._id))
                .map((st) => ({
                    label: st.StoreName,
                    value: st._id
                }))
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mb-6">
                    ...
                    <div>
                        ...
                        {loginVendor.userType == 1 ? <input
                            ...
                            className="w-full px-3 py-2 text-sm md:text-base border rounded-lg bg-gray-100"
                        /> :
                            <Select onChange={(v) => setSelectedStoreFrom(v)} options={storeFrmDrp} isDisabled={loginVendor.userType == 1 ? true : false} value={selectedStoreFrom} />
                        }
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <Select onChange={(v) => setSelectedStoreTo(v)} options={storeToDrp} value={selectedStoreTo} />
                    </div>
                    <div>
                    ...
                    <table className="w-full border-collapse text-sm md:text-base">
                        <thead>
                            <tr className="bg-gray-100">
                                ...
                                <th className="border p-2 hidden lg:table-cell">Reason</th>
