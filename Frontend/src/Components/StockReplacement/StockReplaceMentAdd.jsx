...
const StockReplacementAdd = () => {
    ...
    const getData = async () => {
        ...
        if (AllProducts.length == 0) {
            const productdata = await getDataFundtion("/product")
    ...
    const onSubmit = async (data) => {
        ...
        console.log(data)
        try {
            const data1 = await createDataFunction('/StockReplacement', data)
            toast.success("Data Add")
            setTimeout(()=> {
                navigate('/StockReplacement')
            },2000)
    ...
    const AllCustomer = AllClient.map(item => ({
        label: item.CutomerName,
        value: item._id
    })).slice(0, 50);
    const setDrp = (value) => {
        setReplaceLoction(value)
    ...
    return (
        <div className='mx-5'>
            <ToastContainer />
            <h1 className="text-xl md:text-3xl font-bold text-center text-gray-800 mb-6">Create Stock Replacement</h1>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-4 md:p-6 rounded-lg shadow-md">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <AsyncSelect
                            onChange={(vals) => setCustomerTo(vals)}
                            loadOptions={loadclients}
                            defaultOptions={AllCustomer}
                            value={CustomerTo}
                    ...
                    <div>
                        ...
                        <Select onChange={(v) => setDrp(v)} options={Location} isDisabled={loginVendor.userType == 1 ? true : false} />
                    </div>
                    <div>
                        <label className="block text-sm md:text-base text-gray-700 font-semibold mb-2">Store</label>
                        <Select onChange={(v) => setReplaceStore(v)} options={storeDrp} value={loginVendor.userType == 1 ? selectedStore : ReplaceStore} isDisabled={loginVendor.userType == 1 ? true : false} />
                    </div>
                </div>
...
export default StockReplacementAdd